...
package aze.motion {
	...
	final public class EazeTween
	{
		...
		/**
		 * Has tweens for target
		 * @param	target		Target object
		 */
		static public function hasTweens(target : Object) : Boolean {
			return Boolean(target in running);
		}
		...
		static private function updateTweens(time:int):void 
		{
			...
			while (t)
			{
				...
				{
					if (t._repeat > 0 || t._repeat < 0) 
					{
						const duration : Number = t._duration;

						t.startTime = time;
						t.endTime = time + duration;
						
						if (t._repeat > 0) t._repeat--;
		...
		/**
		 * Set repeat count
		 * @param	repeatCount	Number of repeat of the tween.
		 * @default	-1	Endless repeat 
		 * @return	Tween reference 
		 */
		public function repeat(repeatCount : int = -1) : EazeTween 
		{
			_repeat = repeatCount;
			return this;
		}
		...
		/**
		 * Add a filter animations (PropertyFilter must be activated)
